...
if __name__ == "__main__":
    ...
    if args.target:
        print("Running oneoff scan of - mcp_site_scanner.py:370", args.target)
        ctx = run_scan(args.target)
        print("Summary: - mcp_site_scanner.py:372", json.dumps(ctx.get("summary", {}), indent=2))
        print("Report written to - mcp_site_scanner.py:373", REPORT_FILE)
        if args.once:
            exit(0)
    # otherwise run minimal HTTP tool server for MCP clients
    print("Starting MCP Site Scanner tool server on {}:{} - mcp_site_scanner.py:377".format(args.host, args.port))
